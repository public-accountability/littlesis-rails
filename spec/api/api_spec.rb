describe Api do
  describe 'api_base' do
    it 'returns meta hash by default' do
      expect(Api.send(:api_base, TestActiveRecord.new)).to eql Api::META_HASH
    end

    it 'returns empty hash if meta is false' do
      expect(Api.send(:api_base, TestActiveRecord.new, meta: false)).to eq({})
    end
    ...
    context 'when called with a paginatable_collection' do
      it 'returns meta hash with paginate info by default' do
        ...
        expect(Api.send(:api_base, TestActiveRecord.new)['meta'])
      ...
      it 'returns meta hash with paginate info WITHOUT common metadat info' do
        ...
        expect(Api.send(:api_base, TestActiveRecord.new, meta: false)['meta'])
