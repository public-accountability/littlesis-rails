feature "Editing Tags", :tag_helper, type: :feature do
  ...
  feature 'Admins can edit tag attributes' do
    ...
    it 'displays a form with the tag information' do
      ...
      expect(page).to have_selector 'form[data-controller="tag-form"]'
    ...
    scenario 'Admin can change the description of the tag' do
      ...
      find('form[data-controller="tag-form"] input[name="commit"]').click
    ...
    scenario 'Admin tries to change the tag name to a name that already exists', js: true do
      expect(page).not_to have_selector '.parsley-tagNames'
      expect(Tag.find(tag.id).name).to eq 'oil'
      fill_in('Name', with: 'finance')
      find('form[data-controller="tag-form"] input[name="commit"]').click
      expect(page).to have_selector '.parsley-tagNames'
      expect(Tag.find(tag.id).name).to eq 'oil'
    end
