...
class User
  class Role < Set
    ...
    USER = new('user', %i[login create_map create_list suggest_changes])
    EDITOR = new('editor', USER.dup.merge(%i[edit_database upload_image star_relationship datasets]))
    COLLABORATOR = new('collaborator', EDITOR.dup.merge(%i[bulk_upload match_donations create_tag merge_entity edit_list]))
